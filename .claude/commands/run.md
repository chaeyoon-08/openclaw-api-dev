...
1. **기존 프로세스 정리**
   - 포트 8080, 18789 점유 프로세스 kill
   - openclaw, proxy.js 프로세스 종료
...
2. **Access Token 발급** (SPEC.md 인증 방식 참조)
   ...
   ```
   - `openclaw.json` env.GOG_ACCESS_TOKEN도 python3으로 동적 업데이트
...
4. **exec-approvals.json 설정**
   - `~/.openclaw/exec-approvals.json` 생성

5. **proxy.js 백그라운드 기동**
   - `node proxy.js` 백그라운드 실행, 로그는 `~/.openclaw/proxy.log`
