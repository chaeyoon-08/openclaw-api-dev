#!/bin/bash
# =============================================================
# openclaw-api-dev / run.sh
# 서비스 기동 스크립트 (proxy.js + openclaw gateway)
#
...
pgrep -f 'openclaw|node.*proxy' > /dev/null 2>&1 && sleep 2 || true
...
echo ""
echo "  서비스 종료:"
echo "  pkill -f openclaw; pkill -f 'node.*proxy'"
